...
module.exports = class SerialClient extends EventEmitter{
  ...
  async send(robotId, robotState, linearSpeed, angularSpeed) {
    try {
      angularSpeed = Math.round(angularSpeed)
      linearSpeed = Math.round(linearSpeed)
      if (this.options.debug)
        console.log('>', robotId, robotState, linearSpeed, angularSpeed)
        
      let command = ':'+robotId+';'+robotState+';'+linearSpeed+';'+angularSpeed
      command += '\n'
